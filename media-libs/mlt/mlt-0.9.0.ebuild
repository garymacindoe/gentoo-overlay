...
src_prepare() {
	...
	epatch "${FILESDIR}/${P}-freetype251.patch"
	epatch "${FILESDIR}/${P}-swig-ruby-incdir-and-libs.patch"
	epatch_user
}
...
src_install() {
	...
	if use ruby; then
		...
		exeinto $("${EPREFIX}"/usr/bin/ruby -r rbconfig -e 'print RbConfig::CONFIG["sitearchdir"]')
		doexe mlt.so
